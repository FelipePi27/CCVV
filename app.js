...
function efectoHabilidades(){
    ...
    if(distancia_skills>=300 && entro==false){
        ...
        const intervalFotografia = setInterval(function(){
            pintarBarra(fotografia, 17, 0, intervalFotografia);
        },100);
        const intervalIlustrator = setInterval(function(){
            pintarBarra(ilustrator, 10, 1, intervalIlustrator);
        },100);
        const intervalPython = setInterval(function(){
            pintarBarra(python, 8, 2, intervalPython);
        },100);
        const intervalPhotoshop = setInterval(function(){
            pintarBarra(photoshop, 14, 3, intervalPhotoshop);
        },100);
        const intervalHtml = setInterval(function(){
            pintarBarra(html, 8, 4, intervalHtml);
        },100);
        const intervalWord = setInterval(function(){
            pintarBarra(word, 12, 5, intervalWord);
        },100);
    }
}
